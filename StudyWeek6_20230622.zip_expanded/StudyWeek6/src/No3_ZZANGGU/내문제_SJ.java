...
public class 내문제_SJ {
	...
	public static void main(String[] args) {
		...
		int[] arr2 = new int[list.size()]; // 점수를 담아서 정렬해줄 배열
		...
		for(int i = arr2.length-1 ; i >=0 ; i--) { // 마지막 인덱스가 가장 높은 점수
			for(Map.Entry<String, Integer> map2 : map.entrySet()) {
				if(map2.getValue() == arr2[i]){
					System.out.println(map2.getKey() + "는 " +map2.getValue() + "점입니다! " + (Math.abs(i-arr2.length)) + "등이네요" );
					System.out.println("----------------");
				}
			}
		}
